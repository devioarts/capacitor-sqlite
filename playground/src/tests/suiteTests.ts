...
export function buildSuiteTests(CapacitorSqlite: CapacitorSqlitePlugin): TestCase[] {
  ...
  {
    id: 'trg-06', group: 'Triggers', name: 'Trigger body containing a CASE WHEN...END expression',
    fn: async () => {
      const DB = 'suite_trg06';
      await silentClose(DB);
      assertOk(await CapacitorSqlite.open({ database: DB }), 'open');
      const ddl = await CapacitorSqlite.execute({ database: DB, statements: [
        'DROP TABLE IF EXISTS orders_trg06', 'DROP TABLE IF EXISTS order_log_trg06',
        'DROP TRIGGER IF EXISTS trg_case_log',
        'CREATE TABLE orders_trg06 (id INTEGER PRIMARY KEY, amount REAL)',
        'CREATE TABLE order_log_trg06 (order_id INTEGER, tier TEXT)',
        "CREATE TRIGGER trg_case_log AFTER INSERT ON orders_trg06 BEGIN INSERT INTO order_log_trg06 VALUES (NEW.id, CASE WHEN NEW.amount >= 100 THEN 'large' WHEN NEW.amount >= 10 THEN 'medium' ELSE 'small' END); END",
      ]});
      assertOk(ddl, 'ddl');
      await CapacitorSqlite.run({ database: DB, statement: 'INSERT INTO orders_trg06 VALUES (1, 150)' });
      await CapacitorSqlite.run({ database: DB, statement: 'INSERT INTO orders_trg06 VALUES (2, 50)' });
      await CapacitorSqlite.run({ database: DB, statement: 'INSERT INTO orders_trg06 VALUES (3, 1)' });
      const q = assertOk(await CapacitorSqlite.query({ database: DB, statement: 'SELECT order_id, tier FROM order_log_trg06 ORDER BY order_id' }), 'query');
      assertEqual(q.rows.length, 3, '3 log rows');
      const rows = q.rows as { order_id: number; tier: string }[];
      assertEqual(rows[0].tier, 'large', 'order 1 classified large');
      assertEqual(rows[1].tier, 'medium', 'order 2 classified medium');
      assertEqual(rows[2].tier, 'small', 'order 3 classified small');
      await silentClose(DB);
    },
  },
  {
    id: 'trg-07', group: 'Triggers', name: 'Conditional trigger with a WHEN clause',
    fn: async () => {
      const DB = 'suite_trg07';
      await silentClose(DB);
      assertOk(await CapacitorSqlite.open({ database: DB }), 'open');
      const ddl = await CapacitorSqlite.execute({ database: DB, statements: [
        'DROP TABLE IF EXISTS accounts_trg07', 'DROP TABLE IF EXISTS overdrafts_trg07',
        'DROP TRIGGER IF EXISTS trg_when',
        'CREATE TABLE accounts_trg07 (id INTEGER PRIMARY KEY, balance REAL)',
        'CREATE TABLE overdrafts_trg07 (account_id INTEGER, balance REAL)',
        'CREATE TRIGGER trg_when AFTER UPDATE ON accounts_trg07 WHEN NEW.balance < 0 BEGIN INSERT INTO overdrafts_trg07 VALUES (NEW.id, NEW.balance); END',
      ]});
      assertOk(ddl, 'ddl');
      await CapacitorSqlite.run({ database: DB, statement: 'INSERT INTO accounts_trg07 VALUES (1, 100)' });
      await CapacitorSqlite.run({ database: DB, statement: 'UPDATE accounts_trg07 SET balance = 50 WHERE id = 1' });
      await CapacitorSqlite.run({ database: DB, statement: 'UPDATE accounts_trg07 SET balance = -20 WHERE id = 1' });
      const q = assertOk(await CapacitorSqlite.query({ database: DB, statement: 'SELECT account_id, balance FROM overdrafts_trg07' }), 'query');
      assertEqual(q.rows.length, 1, 'WHEN clause fires trigger only for the negative-balance update');
      assertEqual((q.rows[0] as { balance: number }).balance, -20, 'overdraft balance recorded');
      await silentClose(DB);
    },
  },
  {
    id: 'trg-08', group: 'Triggers', name: 'Cascading triggers across two tables',
    fn: async () => {
      const DB = 'suite_trg08';
      await silentClose(DB);
      assertOk(await CapacitorSqlite.open({ database: DB }), 'open');
      const ddl = await CapacitorSqlite.execute({ database: DB, statements: [
        'DROP TABLE IF EXISTS table_a_trg08', 'DROP TABLE IF EXISTS table_b_trg08', 'DROP TABLE IF EXISTS table_c_trg08',
        'DROP TRIGGER IF EXISTS trg_a_to_b', 'DROP TRIGGER IF EXISTS trg_b_to_c',
        'CREATE TABLE table_a_trg08 (id INTEGER PRIMARY KEY, v TEXT)',
        'CREATE TABLE table_b_trg08 (a_id INTEGER, v TEXT)',
        'CREATE TABLE table_c_trg08 (b_a_id INTEGER, v TEXT)',
        'CREATE TRIGGER trg_a_to_b AFTER INSERT ON table_a_trg08 BEGIN INSERT INTO table_b_trg08 VALUES (NEW.id, NEW.v); END',
        'CREATE TRIGGER trg_b_to_c AFTER INSERT ON table_b_trg08 BEGIN INSERT INTO table_c_trg08 VALUES (NEW.a_id, NEW.v); END',
      ]});
      assertOk(ddl, 'ddl');
      await CapacitorSqlite.run({ database: DB, statement: "INSERT INTO table_a_trg08 VALUES (1, 'chain')" });
      const qb = assertOk(await CapacitorSqlite.query({ database: DB, statement: 'SELECT COUNT(*) AS n FROM table_b_trg08' }), 'query b');
      assertEqual((qb.rows[0] as { n: number }).n, 1, 'first trigger fired');
      const qc = assertOk(await CapacitorSqlite.query({ database: DB, statement: 'SELECT b_a_id, v FROM table_c_trg08' }), 'query c');
      assertEqual(qc.rows.length, 1, 'second trigger fired via cascade');
      assertEqual((qc.rows[0] as { v: string }).v, 'chain', 'value propagated through both triggers');
      await silentClose(DB);
    },
  },
  {
    id: 'trg-09', group: 'Triggers', name: 'Identifiers containing begin/end/case as substrings do not confuse the statement guard',
    fn: async () => {
      const DB = 'suite_trg09';
      await silentClose(DB);
      assertOk(await CapacitorSqlite.open({ database: DB }), 'open');
      const ddl = await CapacitorSqlite.execute({ database: DB, statements: [
        'DROP TABLE IF EXISTS events_trg09', 'DROP TABLE IF EXISTS events_trg09_log',
        'DROP TRIGGER IF EXISTS trg_events_log',
        'CREATE TABLE events_trg09 (id INTEGER PRIMARY KEY, begin_date TEXT, end_date TEXT, usecase TEXT)',
        'CREATE TABLE events_trg09_log (event_id INTEGER, note TEXT)',
        "CREATE TRIGGER trg_events_log AFTER INSERT ON events_trg09 BEGIN INSERT INTO events_trg09_log VALUES (NEW.id, NEW.begin_date || ' to ' || NEW.end_date || ' (' || NEW.usecase || ')'); END",
      ]});
      assertOk(ddl, 'ddl — table/column names containing BEGIN/END/CASE substrings must not be mistaken for the keywords');
      await CapacitorSqlite.run({ database: DB, statement: "INSERT INTO events_trg09 VALUES (1, '2026-01-01', '2026-01-02', 'demo')" });
      const q = assertOk(await CapacitorSqlite.query({ database: DB, statement: 'SELECT note FROM events_trg09_log' }), 'query');
      assertEqual(q.rows.length, 1, 'trigger fired despite begin_date/end_date/usecase column names');
      assertEqual((q.rows[0] as { note: string }).note, '2026-01-01 to 2026-01-02 (demo)', 'note built from correct columns');
      await silentClose(DB);
    },
  },
